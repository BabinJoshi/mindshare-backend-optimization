...
BEGIN
    ...
    FOR rec IN EXECUTE v_query LOOP
        ...
        v_count := v_count + 1;
        IF p_log_every > 0 AND v_count % p_log_every = 0 THEN
            PERFORM mindshare_score._decay_log(p_run_id,'global:TEST',NULL,'running','writing',
...
CREATE OR REPLACE FUNCTION mindshare_score._decay_apply_project_new_replies_test(
    p_project_keyword text,
    p_reset_interval  interval,
    p_run_id          bigint,
    p_log_every       integer
...
BEGIN
    ...
    PERFORM mindshare_score._decay_log(v_run_id,'global:TEST',NULL,'running','init',
    ...
    PERFORM mindshare_score._decay_log(v_run_id,'global:TEST',NULL,'success','done',
        format('Completed (%s): %s repliers recomputed, %s rows written (TEST TABLE)',
               CASE WHEN v_dirty IS NULL THEN 'full rebuild (first run)' ELSE 'incremental' END,
               COALESCE(v_dirty::text,'ALL'), v_count), v_count, NULL,NULL,NULL,NULL, true);
    RETURN v_run_id;
...
EXCEPTION WHEN OTHERS THEN
    ...
    BEGIN
        GET STACKED DIAGNOSTICS v_detail = PG_EXCEPTION_DETAIL, v_context = PG_EXCEPTION_CONTEXT;
        PERFORM mindshare_score._decay_log(v_run_id,'global:TEST',NULL,'failed','error',
            format('FAILED (incremental, TEST TABLE): %s', v_msg), v_count, v_state, v_msg, v_detail, v_context, true);
    END;
    RAISE;
END;
...
-- SELECT COUNT(*) as prod_count FROM mindshare_score.contribution_scores_test WHERE project_keyword='default';
--
-- -- Once validated, switch to test tables by using the original functions
-- SELECT mindshare_score.calculate_decay_scores_incremental_test('default', '30 days'::interval);
-- ============================================================================
