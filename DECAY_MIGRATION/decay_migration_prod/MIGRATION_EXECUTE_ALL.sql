-- ============================================================================
-- MIGRATION_EXECUTE_ALL.sql
-- Master script to replicate incremental decay from test to prod schemas
-- BUT writes to production score tables for validation before going live
--
-- Differences from MIGRATION_EXECUTE_ALL.sql:
--   - Writes to: contribution_scores, global_contribution_scores
...
--   2. Validate results in _production tables
...
CREATE TABLE IF NOT EXISTS mindshare_score.contribution_scores (
...
CREATE TABLE IF NOT EXISTS mindshare_score.global_contribution_scores (
    reply_post_id         text        NOT NULL,
    original_post_id      text        NOT NULL,
    replier_x_id          text        NOT NULL,
    original_author_x_id  text        NOT NULL,
...
-- Create indexes on production tables
CREATE INDEX IF NOT EXISTS ix_cs_keyword_orig_replier_time
...
-- ============================================================================
-- PHASE 4: CORE DECAY FUNCTIONS (WRITES TO _PRODUCTION TABLES)
-- ============================================================================

CREATE OR REPLACE FUNCTION mindshare_score._decay_apply_project(
...
CREATE OR REPLACE FUNCTION mindshare_score._decay_apply_global(
    p_reset_interval interval,
    p_run_id         bigint,
    p_only_dirty     boolean,
    p_log_every      integer
...
-- ============================================================================
-- PHASE 5: TAIL CORE FUNCTIONS (WRITES TO _PRODUCTION TABLES)
-- ============================================================================

CREATE OR REPLACE FUNCTION mindshare_score._decay_apply_project_tail(
    p_project_keyword text,
    p_reset_interval  interval,
    p_run_id          bigint,
    p_log_every       integer
...
-- ============================================================================
-- OPTIMIZED: Handle new replies without full history replay
-- ============================================================================

CREATE OR REPLACE FUNCTION mindshare_score._decay_apply_project_new_replies(
    p_project_keyword text,
    p_reset_interval  interval,
    p_run_id          bigint,
    p_log_every       integer
...
BEGIN
    ...
    LOOP
        ...
        IF p_log_every > 0 AND v_count % p_log_every = 0 THEN
            PERFORM mindshare_score._decay_log(p_run_id,'project',p_project_keyword,'running','writing',
                format('%s new reply scores written...', v_count), v_count);
        END IF;
    END LOOP;
    RETURN v_count;
END;
...
CREATE OR REPLACE FUNCTION mindshare_score._decay_apply_global_tail(
    p_reset_interval interval,
    p_run_id         bigint,
    p_log_every      integer
) RETURNS bigint
...
-- ============================================================================
-- PHASE 6: INCREMENTAL ENTRY POINTS (WRITES TO _PRODUCTION TABLES)
-- ============================================================================

CREATE OR REPLACE FUNCTION mindshare_score.calculate_decay_scores_incremental(
    p_project_keyword text,
...
BEGIN
    ...
    PERFORM mindshare_score._decay_log(v_run_id,'project',p_project_keyword,'running','init',
        format('INCREMENTAL decay run started (project=%s, PRODUCTION TABLE, reset_interval=%s)', p_project_keyword, p_reset_interval), 0);
    ...
    IF v_since IS NULL THEN
        PERFORM mindshare_score._decay_log(v_run_id,'project',p_project_keyword,'running','computing',
            'First run: full rebuild for project (PRODUCTION TABLE)', 0);
        DELETE FROM mindshare_score.contribution_scores WHERE project_keyword = p_project_keyword;
        v_count := mindshare_score._decay_apply_project(p_project_keyword, p_reset_interval, v_run_id, false, p_log_every);
        v_dirty := NULL;
    ELSE
        -- Detect changes
        ...
        PERFORM mindshare_score._decay_log(v_run_id,'project',p_project_keyword,'running','computing',
            format('%s users with profile changes (full replay) + new replies from others - PRODUCTION TABLE', v_dirty), 0);

        -- Process profile-changed users: full history replay
        IF (SELECT COUNT(*) FROM tmp_dirty) > 0 THEN
            DELETE FROM mindshare_score.contribution_scores cs
            USING tmp_dirty d
            WHERE cs.project_keyword = p_project_keyword
              AND cs.replier_x_id = d.replier_x_id
              AND cs.post_created_at >= d.t_min;

            v_count := mindshare_score._decay_apply_project_tail(
        ...
        v_count := v_count + mindshare_score._decay_apply_project_new_replies(
                       p_project_keyword, p_reset_interval, v_run_id, p_log_every);
    END IF;
    ...
    PERFORM mindshare_score._decay_log(v_run_id,'project',p_project_keyword,'success','done',
        format('Completed (%s): %s repliers recomputed, %s rows written (PRODUCTION TABLE)',
...
CREATE OR REPLACE FUNCTION mindshare_score.calculate_global_decay_scores_incremental(
    p_reset_interval interval DEFAULT '30 days',
    p_run_id         bigint   DEFAULT NULL,
    p_log_every      integer  DEFAULT 50000
) RETURNS bigint
...
BEGIN
    ...
    PERFORM mindshare_score._decay_log(v_run_id,'global',NULL,'running','init',
        format('INCREMENTAL global decay run started (PRODUCTION TABLE, reset_interval=%s)', p_reset_interval), 0);
    ...
    IF v_since IS NULL THEN
        PERFORM mindshare_score._decay_log(v_run_id,'global',NULL,'running','computing',
            'First run: full global rebuild (PRODUCTION TABLE)', 0);
        TRUNCATE TABLE mindshare_score.global_contribution_scores;
        v_count := mindshare_score._decay_apply_global(p_reset_interval, v_run_id, false, p_log_every);
        v_dirty := NULL;
    ELSE
        DROP TABLE IF EXISTS tmp_changed;
        CREATE TEMP TABLE tmp_changed ON COMMIT DROP AS
        ...
        PERFORM mindshare_score._decay_log(v_run_id,'global',NULL,'running','computing',
            format('%s dirty repliers to recompute (tail-from-t_min) - PRODUCTION TABLE', v_dirty), 0);

        IF v_dirty > 0 THEN
            DELETE FROM mindshare_score.global_contribution_scores cs
            USING tmp_dirty d
            WHERE cs.replier_x_id = d.replier_x_id
              AND cs.post_created_at >= d.t_min;

            v_count := mindshare_score._decay_apply_global_tail(p_reset_interval, v_run_id, p_log_every);
        END IF;
    END IF;
...
BEGIN
    ...
    RAISE NOTICE 'Run calculate_all_global_decay_scores_incremental() separately for global decay';
    RAISE NOTICE '════════════════════════════════════════════════════';
END;
$func$;
...
SELECT 'Migration to TEST tables complete!'::text as status,
       (SELECT COUNT(*) FROM information_schema.tables
        WHERE table_schema='mindshare_score' AND table_name IN ('contribution_scores','global_contribution_scores')) as test_tables_created,
       (SELECT COUNT(*) FROM information_schema.routines
        WHERE routine_schema='mindshare_score' AND routine_name LIKE '%test%') as test_functions_created;

-- ============================================================================
-- NEXT STEPS: Execute Phase 7 (First Run - PRODUCTION TABLES)
-- ============================================================================
-- To run incremental decay on production data but write to production tables:
--
...
-- SELECT COUNT(*) as test_count FROM mindshare_score.contribution_scores WHERE project_keyword='default';
