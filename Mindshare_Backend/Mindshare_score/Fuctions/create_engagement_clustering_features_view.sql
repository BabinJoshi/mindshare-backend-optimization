-- DROP PROCEDURE mindshare_score.create_engagement_clustering_features_view(text);

CREATE OR REPLACE PROCEDURE mindshare_score.create_engagement_clustering_features_view(IN project_keyword text)
 LANGUAGE plpgsql
AS $procedure$
DECLARE
...
BEGIN
    ...
    EXECUTE format($sql$
        ...
        root_stats AS (
            SELECT
                ...
                -- P90 timestamp to ignore straggler outliers for duration penalty
                TO_TIMESTAMP(PERCENTILE_CONT(0.90) WITHIN GROUP (ORDER BY EXTRACT(EPOCH FROM engaged_tweet_created_at))) AS p90_engagement
            FROM base
            GROUP BY 1, 2, 3, 4
        ),

        -- 2️⃣ Burst concentration (60 min window) — O(N log N) via window function
        burst_windows AS (
            SELECT
                root_post_id,
        ...
        -- 5️⃣ Cross-post overlap (OPTIMIZED: Window functions instead of massive cross-joins)
        ...
        SELECT
            ...
            0 AS prev_post_overlap, -- Left for backward compatibility, unused in service
    ...
    EXECUTE format('CREATE UNIQUE INDEX IF NOT EXISTS %I ON mindshare_score.%I (root_post_id)', index_name, features_view_name);
END;
$procedure$
;