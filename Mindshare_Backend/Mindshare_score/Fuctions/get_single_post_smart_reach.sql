-- DROP FUNCTION mindshare_score.get_single_post_smart_reach(text, text, int8, int8);

CREATE OR REPLACE FUNCTION mindshare_score.get_single_post_smart_reach(target_post_id text, projectname text, startdate bigint DEFAULT NULL::bigint, enddate bigint DEFAULT NULL::bigint)
 RETURNS TABLE(post_id text, handle text, username character varying, post_unique_reach numeric, smart_reach numeric, like_reply_ratio numeric, impression_to_unique_reach_ratio numeric, unique_engager_scores_to_count_ratio numeric, like_reply_engagement_farming_flag boolean, like_reply_botting_flag boolean, impression_unique_reach_engagement_farming_flag boolean, impression_unique_reach_botting_flag boolean)
 LANGUAGE plpgsql
 STABLE
AS $function$
DECLARE
...
WITH engagements AS (
    SELECT
        ...
        root_reply_count
    FROM analytics.%I
    WHERE ($1 IS NULL OR engaged_tweet_created_at >= to_timestamp($1))
      AND ($2 IS NULL OR engaged_tweet_created_at <= to_timestamp($2))
      AND ($1 IS NULL OR root_tweet_created_at   >= to_timestamp($1))
      AND ($2 IS NULL OR root_tweet_created_at   <= to_timestamp($2))
...
ratio_data AS (
    ...
    JOIN mindshare.mindshare_post p
       ...
       AND p.project_keyword = $4
),

decay_implemented_data AS (
    SELECT
        cs.original_post_id,
        cs.replier_x_id,
        cs.post_created_at,
    ...
    WHERE cs.original_post_id = $3
      AND ($1 IS NULL OR cs.post_created_at >= to_timestamp($1))
      AND ($2 IS NULL OR cs.post_created_at <= to_timestamp($2))
      AND cs.replier_x_id <> cs.original_author_x_id
),

unique_engager AS (
    SELECT DISTINCT ON (original_post_id, replier_x_id)
        original_post_id,
        contribution_score
    FROM decay_implemented_data
    ORDER BY original_post_id, replier_x_id, post_created_at ASC
),
...
FROM ratio_data r
LEFT JOIN mindshare.mindshare_user u ON r.handle = u.x_id
LEFT JOIN smart_reach_cte sr ON sr.original_post_id = r.post_id
$q$, view_name);

RETURN QUERY EXECUTE sql_query USING startdate, enddate, target_post_id, projectname;
