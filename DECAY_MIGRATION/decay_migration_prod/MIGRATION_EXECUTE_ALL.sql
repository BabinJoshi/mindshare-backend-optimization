...
-- ============================================================================
-- OPTIMIZED: Handle new global replies without full history replay
-- ============================================================================

CREATE OR REPLACE FUNCTION mindshare_score._decay_apply_global_new_replies(
    p_reset_interval interval,
    p_run_id         bigint,
    p_log_every      integer
) RETURNS bigint
...
BEGIN
    ...
    ELSE
        ...
        FROM tmp_changed c
        LEFT JOIN mindshare_score.contribution_scores cs
        ...
        PERFORM mindshare_score._decay_log(v_run_id,'project',p_project_keyword,'running','computing',
            format('Processing new replies only (profile changes excluded) - PRODUCTION TABLE'), 0);

        -- Process new replies: directly calculate and insert (OPTIMIZED - no history replay)
        v_count := mindshare_score._decay_apply_project_new_replies(
...
BEGIN
    ...
    ELSE
        ...
        SELECT c.post_id, c.user_x_id, c.replied_post_id, c.post_created_at
        FROM tmp_changed c
        LEFT JOIN mindshare_score.global_contribution_scores cs
        ...
        -- Process new replies: directly calculate and insert (OPTIMIZED - no history replay)
        v_count := mindshare_score._decay_apply_global_new_replies(p_reset_interval, v_run_id, p_log_every);
    END IF;
