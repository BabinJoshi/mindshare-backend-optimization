-- ============================================================================
-- decay_03_calculate_global_decay_scores.sql  (GLOBAL decay: shared core + full rebuild)
--   * _decay_apply_global(...)            -- per-replier decay loop over user_post (one copy).
--   * calculate_global_decay_scores(...)  -- FULL rebuild wrapper (truncate + replay everyone).
--   The INCREMENTAL wrapper lives in decay_12_*.sql and reuses this core.
-- ============================================================================

-- ---------------------------------------------------------------------------
-- CORE: per-replier rolling-window decay loop on user_post.
--   p_only_dirty = true -> restricted to tmp_dirty_repliers(replier_x_id).
--   Returns rows written. May RAISE (caller handles).
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION test_mindshare_score._decay_apply_global(
...
BEGIN
    v_query :=
        ...
        'FROM test_mindshare.user_post p '
        'INNER JOIN test_mindshare.user_post op ON p.replied_post_id = op.post_id '
    ...
    END IF;
    -- deterministic tiebreaker (see decay_02): without post_id, replies at the same
    -- (user_x_id, post_created_at) order non-deterministically -> incremental != full.
...
-- ---------------------------------------------------------------------------
-- FULL rebuild wrapper (truncate + replay everyone; delegates loop to the core)
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION test_mindshare_score.calculate_global_decay_scores(
...
BEGIN
    PERFORM test_mindshare_score._decay_log(v_run_id,'global',NULL,'running','init',
        format('FULL global decay run started (reset_interval=%s)', p_reset_interval), 0);

    PERFORM test_mindshare_score._decay_log(v_run_id,'global',NULL,'running','clearing',
        'Truncating global_contribution_scores', 0);
